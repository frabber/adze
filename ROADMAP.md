...
- [ ] M0.1 Workspace skeleton (D18), CI on three platforms, first property test
- [ ] M0.2 Spike: persistent mesh storage vs mutable BMesh, 1M faces, query cost (D5)
- [ ] M0.3 Spike: integer-lattice coordinates, rotation rounding policy, unit model (D6)
- [ ] M0.4 Determinism harness: same op sequence hashed on all platforms and wasm (D7)
- [ ] M0.5 Research notes: BMesh/OpenMesh comparison, OpenSubdiv crease rules

## M1. Mesh kernel
Exit: OBJ round-trips on a corpus of 50 real meshes; property tests green for 10k random op sequences.
- [ ] Topology: verts, edges, faces (n-gons), non-manifold tolerated, stable IDs
- [ ] Typed attribute columns: position, normal, UV, crease, groups, custom
- [ ] Invariant checker (manifoldness, orphan elements, attribute lengths)
- [ ] OBJ import/export (D8)
- [ ] Primitives: plane, cube, cylinder, sphere, torus
- [ ] Headless viewer (native only) rendering wireframe + shaded, for eyeballing
...
- [ ] Selection modes: vertex, edge, face; loops, rings, grow/shrink, invert
- [ ] Selection stored as rules + concrete IDs (survives topology change)
- [ ] Tool pipe traits: action × action center × falloff × symmetry × snapping
- [ ] Transform tools with in-viewport handles
- [ ] Op log with snapshots; undo/redo via replay (D2)
- [ ] Autosave of op log (native: file; web: origin private file system)

## M3. Core ops
Exit: a hard-surface prop (e.g. a hinge) can be modeled start to finish.
- [ ] Extrude, inset, loop cut, knife, bridge, merge, dissolve, edge slide, connect
- [ ] Differential tests against Blender headless for each op (D13)
- [ ] Web build (wasm + WebGPU, WebGL2 fallback) with the same tools
- [ ] Artifact-hosted demo writing op logs and comments to its database
...
- [ ] Bevel with deterministic output topology and stable derived IDs
- [ ] Catmull-Clark on the GPU matching OpenSubdiv creases (D9)
- [ ] Symmetry, snapping, workplane
- [ ] Geodesic (heat-method) falloff on the GPU
- [ ] First testers on the web demo; op-log review sessions
...
- [ ] Live ops with incremental re-evaluation (salsa or equivalent)
- [ ] Fail-soft reference loss: evaluate with found elements, highlight missing, repick
- [ ] Delta layers for post-op tweaks, re-projected on topology change
- [ ] Squash history range into a baked op
- [ ] Boolean research and implementation (D16)
- [ ] Crease-to-holding-edge derivation
